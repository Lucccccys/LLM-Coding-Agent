...
3. Before any `str_replace`, use `view` on the target file in the same turn window and copy `old_str` exactly from observed file content
4. Witness requirement for edits: before any edit command (`str_replace`, `insert`, `undo_edit`), you must have a witness from tool output (recent `view` snippet and/or explicit path existence check) for the exact file and region you are about to modify
5. Pre-edit verification: right before an edit, verify (a) path exists, (b) target region still matches latest `view`, and (c) `old_str` is exact and unique
...
4. FAILED REPLACEMENT RECOVERY:
    - If you receive `No replacement was performed`, do not retry the same `old_str`.
    - Re-run `view` around the intended location, then rebuild `old_str` with exact and unique context.
    - Enter recovery mode for the next step: stop editing attempts, re-establish witness via `view`, then attempt one corrected edit.
    - Do not invent stack traces, error messages, or guessed code as `old_str`.
...
* Always call `view` first and copy `old_str` from the observed file content
* Edit witness is mandatory: do not edit a file/region unless it was observed in recent tool output
* The `old_str` parameter should match EXACTLY one or more consecutive lines from the original file. Be mindful of whitespaces!
* If the `old_str` parameter is not unique in the file, the replacement will not be performed. Make sure to include enough context in `old_str` to make it unique
* The `new_str` parameter should contain the edited lines that should replace the `old_str`
* If replacement fails, enter recovery mode: do not repeat the same `old_str`; re-`view`, verify path/region again, then rebuild `old_str` with exact context
"""
