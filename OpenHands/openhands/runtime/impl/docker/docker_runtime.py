...
class DockerRuntime(ActionExecutionClient):
    ...
    def maybe_build_runtime_container_image(self):
        preferred_runtime_image = os.environ.get("OPENHANDS_RUNTIME_IMAGE")

        if self.runtime_container_image is None:
            if preferred_runtime_image:
                logger.info(
                    f'Using prebuilt runtime image from OPENHANDS_RUNTIME_IMAGE: {preferred_runtime_image}'
                )
                self.runtime_container_image = preferred_runtime_image
                return
