...
from openhands.controller.search_loop_detector import SearchLoopDetector
...
PATH_RECOVERY_HINT = (
    '\n\n[Path Recovery Hint]\n'
    'The previous command failed due to an invalid or missing path. '
    'Before retrying, first run `pwd` to confirm the current directory, '
    'then use `ls` or `find` to locate the correct path. '
    'Avoid repeating the same command until the path is verified.'
)

EMPTY_SEARCH_HINT = (
    '\n\n[Empty Search Recovery Hint]\n'
    'The search returned no results. Do NOT repeat a similar search. '
    'Instead: (1) verify the search target exists by listing candidate files, '
    '(2) broaden the search to a single key token, or (3) read the file directly with `cat`.'
)

SYNTAX_ERROR_HINT = (
    '\n\n[Syntax Error Recovery Hint]\n'
    'The command had a syntax error. Do NOT retry the same command pattern. '
    'Common fix: avoid `find -exec ... | grep ...` (pipes do not work inside -exec). '
    'Instead use a plain grep: `grep -rn "keyword" . --include="*.py"`'
)
...
class AgentController:
    ...
    def __init__(
        ...
        self._search_loop_detector = SearchLoopDetector()
    ...
    async def _handle_observation(self, observation: Observation) -> None:
        ...
        """
        self._maybe_add_path_recovery_hint(observation)
        self._maybe_add_search_recovery_hint(observation)
    ...
    def _maybe_add_path_recovery_hint(self, observation: Observation) -> None:
        """Append a recovery hint to path-not-found command outputs.

        This nudges the agent to verify/locate paths before retrying.
        """
        if not self._pending_action:
            return
        if self._pending_action.id != observation.cause:
            return
        if not isinstance(self._pending_action, CmdRunAction):
            return
        if not hasattr(observation, 'content') or not observation.content:
            return

        content = observation.content
        if any(needle in content for needle in PATH_NOT_FOUND_NEEDLES):
            if PATH_RECOVERY_HINT not in content:
                observation.content = content + PATH_RECOVERY_HINT

    def _maybe_add_search_recovery_hint(self, observation: Observation) -> None:
        """Detect repeated failed searches and inject corrective hints.

        Uses the SearchLoopDetector to track search commands and their
        outcomes. When a pattern of repeated failures is detected, a
        recovery hint is appended to the observation content.
        """
        if not self._pending_action:
            return
        if self._pending_action.id != observation.cause:
        ...
        hint = self._search_loop_detector.record_search_result(
            command, content, exit_code
        )
        if hint and hint not in content:
            observation.content = content + hint

    def _track_action_for_loop_prevention(self, action: Action) -> None:
        """Track whether the agent is taking concrete vs stall actions.

        Stall actions (think, recall, message) burn iterations without progress.
        When too many consecutive stalls are detected, a corrective hint is
        injected into the event stream.
        """
        is_stall = isinstance(action, (AgentThinkAction, RecallAction, NullAction))
        if isinstance(action, MessageAction) and action.source == EventSource.AGENT:
            is_stall = True

        if is_stall:
            hint = self._search_loop_detector.record_stall_action(
                type(action).__name__
            )
            if hint:
                self.event_stream.add_event(
                    ErrorObservation(content=hint),
                    EventSource.AGENT,
                )
        else:
            self._search_loop_detector.record_concrete_action()
    ...
    async def _step(self) -> None:
        ...
        self._track_action_for_loop_prevention(action)
