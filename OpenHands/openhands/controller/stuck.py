...
class StuckDetector:
    ...
    def is_stuck(self, headless_mode: bool = True) -> bool:
        ...
        # scenario 6: fuzzy search loop — same search target repeated across
        # slightly different commands (e.g. grep target stays the same but
        # flags/paths vary), all producing empty or error results
        if len(filtered_history) >= 6:
            if self._is_stuck_search_target_loop(
                filtered_history, filtered_history_offset
            ):
                return True
    ...
    def _is_stuck_search_target_loop(
        self, filtered_history: list[Event], filtered_history_offset: int = 0
    ) -> bool:
        """Detect loops where the agent searches for the same target repeatedly
        with slightly different commands, all producing empty/error results.

        Unlike scenario 1 (exact action match), this uses fuzzy matching:
        it extracts the search target from grep/find commands and checks
        whether the same target has been searched 4+ times with no results.
        """
        extractor = SearchLoopDetector()

        recent_cmd_actions: list[tuple[int, Event]] = []
        recent_cmd_observations: list[tuple[int, Event]] = []

        for i, event in enumerate(filtered_history):
            if isinstance(event, CmdRunAction):
                recent_cmd_actions.append((i, event))
            elif isinstance(event, CmdOutputObservation):
                recent_cmd_observations.append((i, event))

        if len(recent_cmd_actions) < 4:
            return False

        target_fail_counts: dict[str, list[int]] = {}

        for idx, action in recent_cmd_actions:
            if not isinstance(action, CmdRunAction):
                continue
            target = extractor.extract_search_target(action.command)
            if target is None:
                continue

            obs_for_action = None
            for obs_idx, obs in recent_cmd_observations:
                if obs_idx > idx:
                    obs_for_action = obs
                    break

            if obs_for_action is None:
                continue

            content = obs_for_action.content if hasattr(obs_for_action, 'content') else ''
            exit_code = -1
            if isinstance(obs_for_action, CmdOutputObservation):
                if hasattr(obs_for_action, 'metadata') and obs_for_action.metadata:
                    exit_code = obs_for_action.metadata.exit_code
                elif hasattr(obs_for_action, 'exit_code'):
                    exit_code = obs_for_action.exit_code

            is_failure = (
                (content.strip() == '' and exit_code != 0)
                or 'syntax error' in content.lower()
                or 'unexpected token' in content.lower()
            )

            if is_failure:
                target_lower = target.lower()
                if target_lower not in target_fail_counts:
                    target_fail_counts[target_lower] = []
                target_fail_counts[target_lower].append(idx)

        for target, indices in target_fail_counts.items():
            if len(indices) >= 4:
                logger.warning(
                    f'Fuzzy search loop detected: target "{target}" failed '
                    f'{len(indices)} times across different commands'
                )
                self.stuck_analysis = StuckDetector.StuckAnalysis(
                    loop_type='repeating_search_target',
                    loop_repeat_times=len(indices),
                    loop_start_idx=indices[0] + filtered_history_offset,
                )
                return True
