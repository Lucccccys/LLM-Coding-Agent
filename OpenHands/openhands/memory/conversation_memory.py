...
class ConversationMemory:
    ...
    @staticmethod
    def _get_cmd_recovery_hint(obs: 'CmdOutputObservation') -> str:
        """Generate a recovery hint for failed or empty command observations.

        Appends actionable guidance directly to the observation text so the
        model sees the hint in context, rather than relying on system-prompt
        rules it may have forgotten.
        """
        import re

        cmd = obs.command.strip()
        content = obs.content.strip()
        exit_code = obs.metadata.exit_code

        # Detect grep/find with regex syntax errors (e.g. "Unmatched ( or \\(")
        if exit_code != 0 and re.search(
            r'Unmatched \(|Unmatched \)|Invalid regexp|unterminated',
            content,
            re.IGNORECASE,
        ):
            return (
                '\n\n[RECOVERY HINT: The search pattern contains unescaped regex '
                'metacharacters. Use `grep -F` for literal string matching, or '
                'properly escape special characters like (, ), [, ], +, ?, {, }, |.]'
            )

        # Detect empty search results from grep/find
        is_search_cmd = any(
            cmd.startswith(prefix) or f' {prefix}' in cmd or f'|{prefix}' in cmd
            for prefix in ['grep', 'find', 'rg', 'ag']
        )
        if is_search_cmd and not content and exit_code != 0:
            return (
                '\n\n[RECOVERY HINT: Search returned no results. Do NOT retry with '
                'a similar pattern. Instead: (1) verify the working directory with '
                '`pwd`, (2) list candidate files with `ls` or `find`, (3) try a '
                'broader/simpler search term, or (4) use `grep -r` with a key '
                'substring instead of the full phrase.]'
            )

        # Detect path-not-found errors
        if exit_code != 0 and re.search(
            r'No such file or directory|cannot access|does not exist',
            content,
            re.IGNORECASE,
        ):
            return (
                '\n\n[RECOVERY HINT: Path not found. Use `pwd` to verify your '
                'working directory, then `find` or `ls` to locate the correct path '
                'before retrying.]'
            )
    ...
    @staticmethod
    def _get_edit_recovery_hint(obs: 'FileEditObservation') -> str:
        """Generate a recovery hint for failed file edit observations."""
        content_str = str(obs.content) if obs.content else ''
        if 'No replacement was performed' in content_str:
            return (
                '\n\n[RECOVERY HINT: The old_str did not match file content. '
                'Do NOT retry with a guessed old_str. Instead: (1) use `view` '
                'on the target file to see its actual content, (2) copy the '
                'exact text from the view output, (3) then retry str_replace '
                'with the verified old_str.]'
            )
        return ''

    @staticmethod
    def _build_failure_pattern_summary(events: list[Event]) -> str:
        """Scan recent events for consecutive failure patterns and build a
        progressive intervention message.

        This addresses a fundamental gap: the StuckDetector only catches exact
        repetitions (threshold 3-4). Semantically similar but non-identical
        failures (e.g., searching 3 different non-existent strings) slip
        through. This method provides softer, earlier intervention.

        Returns an empty string when no concerning pattern is detected.
        """
        import re

        recent_window = 12
        tail = events[-recent_window:] if len(events) > recent_window else events

        consecutive_empty_searches = 0
        consecutive_failed_edits = 0
        consecutive_think_actions = 0

        for event in tail:
            if isinstance(event, CmdOutputObservation):
                cmd = event.command.strip() if hasattr(event, 'command') else ''
                is_search = any(
                    kw in cmd for kw in ['grep', 'find', 'rg', 'ag']
                )
                if is_search and not event.content.strip() and event.error:
                ...
                consecutive_think_actions += 1

            elif isinstance(event, Action) and not isinstance(
                event, AgentThinkAction
            ):
                consecutive_think_actions = 0
        ...
        if consecutive_empty_searches >= 3:
            parts.append(
                f'You have had {consecutive_empty_searches} consecutive empty '
                'search results. STOP refining similar queries. Take a '
                'fundamentally different approach: list files in the repo, '
                'read a likely candidate file directly, or search for a much '
                'shorter/broader keyword.'
            )

        if consecutive_failed_edits >= 2:
            parts.append(
                f'You have had {consecutive_failed_edits} consecutive failed '
                'edits. STOP guessing file content. Run `view` on the target '
                'file RIGHT NOW and copy exact text before attempting another '
                'edit.'
            )

        if consecutive_think_actions >= 3:
            parts.append(
                f'You have called `think` {consecutive_think_actions} times '
                'recently without taking a concrete action. Each call uses '
                'one of your limited iterations. Take a concrete action now '
                '(search, read, or edit).'
            )
    ...
    def process_events(
        ...
        # Inject a progressive failure summary if recent events show a pattern
        failure_summary = self._build_failure_pattern_summary(events)
        if failure_summary and messages:
            last_msg = messages[-1]
            if last_msg.role == 'user' and last_msg.content:
                for item in last_msg.content:
                    if isinstance(item, TextContent):
                        item.text += failure_summary
                        break
    ...
    def _process_observation(
        ...
        if isinstance(obs, CmdOutputObservation):
            ...
            text += self._get_cmd_recovery_hint(obs)
            ...
            text += self._get_edit_recovery_hint(obs)
            message = Message(role='user', content=[TextContent(text=text)])
